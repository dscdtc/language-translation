...
def model_inputs():
    ...
    """
    # TODO: Implement Function
    model_input = tf.placeholder(tf.int32, shape=(None,None), name='input')
    model_target = tf.placeholder(tf.int32, shape=(None,None), name='target')
    lr = tf.placeholder(tf.float32, shape=None, name='lr')
    keep_prob = tf.placeholder(tf.float32, shape=None, name='keep_prob')
    return model_input, model_target, lr, keep_prob
...
def process_decoding_input(target_data, target_vocab_to_int, batch_size):
    ...
    """
    # TODO: Implement Function
    temp = tf.strided_slice(target_data, [0,0], [batch_size, -1], [1,1])
    go = tf.fill([batch_size, 1], target_vocab_to_int['<GO>'])
    target_input = tf.concat([go, temp], 1)
    return target_input
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    mrnn_cell = tf.contrib.rnn.MultiRNNCell([lstm_cell] * num_layers)
    drop_cell = tf.contrib.rnn.DropoutWrapper(mrnn_cell, keep_prob)
    _, encoding_state = tf.nn.dynamic_rnn(drop_cell, rnn_inputs, dtype=tf.float32)
...
def decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
    ...
    """
    # TODO: Implement Function
    d_fn_inference = tf.contrib.seq2seq.simple_decoder_fn_inference(
        output_fn, encoder_state, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
        maximum_length, vocab_size)
    drop_cell  = tf.contrib.rnn.DropoutWrapper(dec_cell, keep_prob)
    inference_logits, _, _ = tf.contrib.seq2seq.dynamic_rnn_decoder(
        drop_cell, d_fn_inference, scope=decoding_scope)
...
def decoding_layer(dec_embed_input, dec_embeddings, encoder_state, vocab_size, sequence_length, rnn_size,
    ...
    lstm_cell   = tf.contrib.rnn.BasicLSTMCell(rnn_size)
    mrnn_cell = tf.contrib.rnn.MultiRNNCell([lstm_cell] * num_layers)
    dec_cell = tf.contrib.rnn.DropoutWrapper(mrnn_cell, keep_prob)
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, sequence_length, source_vocab_size, target_vocab_size,
    ...
    training_logits, inference_logits = decoding_layer(dec_embed_input, dec_embeddings, encoding_state, target_vocab_size,
